...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function hash_password_db($id, $password, $tablename='',$use_sha1_override=FALSE, $i=null)
	{
   ...
   $query = $this->db->select('password, salt')
	 ...
	 * @author Mathew
	 **/
	public function activate($id, $code = false, $i=null)
	{
		...
		else
		{
			...
			$this->trigger_events('extra_where');
			$this->db->update($this->tables['users'], $data, array('customer_user_id' => $id));
	 ...
	 * @author Mathew
	 **/
	public function deactivate($id = NULL, $i=null)
	{
	...
	public function clear_forgotten_password_code($code, $i=null) {
	 ...
	 * @author Mathew
	 **/
	public function reset_password($identity, $new, $i=null) {
		...
		$this->trigger_events('extra_where');
     	$type_of_user = $this->session->userdata('reset_password');
     
       if($type_of_user !='' && $type_of_user =='emp'){
		...
		$query = $this->db->get();
          ...
          $query = $this->db->select('customer_user_id, password, salt')
	    ...
	    if($type_of_user !='' && $type_of_user =='emp'){
	        	$this->trigger_events('extra_where');
			$this->db->update('employee', $data, array($this->identity_column => $identity));
	...
	public function change_password($identity, $old, $new)
	{
		...
		$old_password_matches = $this->hash_password_db($user->customer_user_id, $old);
...
$check = $this->db->where($this->identity_column, $identity)
		                ->count_all_results($this->tables['users']);
		               if($check == 0){
		                   	return $this->db->where('email', $identity)
		                ->count_all_results('employee') > 0;
		               }else{
	...
	public function forgotten_password($identity)
	{
       ...
       if($return == ''){
         ...
         $this->db->update('employee', $update, array($this->identity_column => $identity));
		$return = $this->db->affected_rows() == 1;
       }
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email, $package, $phone = '', $additional_data = array(), $menu_access = array(),$supervisor='')
	{  
		...
		$data = array(
		    ...
		    'password'   => $password,
		    'email'      => $email,
		    'is_haccap_user' => $is_haccap_user,
		    'is_preparea_user' => $is_preparea_user,
		    'phone'      => $phone,
		    'role'      => $package,
		    ...
		    'customer_id' => $cust_id,
		    'branch_id' => $branch_id,
		    'menu_access' => serialize($menu_access),
		    'created_on' => time(),
		    'last_login' => time(),
...
// 		$user_data = array_merge($this->_filter_data($this->tables['customer_users'], $additional_data), $data);
		...
		foreach($additional_data as $branches){
		    ...
		    $this->db->insert('branches_access', $branch_data);
		...
		$this->db->insert($this->tables['users_groups'], $user_type_data);
...
public function emp_register($first_name,$last_name,$username, $password,$user_type,$active,$mobile,$res_id,$Specilized_category, $additional_data = array(), $groups = array())
	{
    ...
    $module_data=array(
	'moduleId'=> substr(implode(',',$Specilized_category), 0),
	'userId'=>$insert_id,
	'empId'=>$res_id
	);

	$this->db->insert('module_access',$module_data);
		}
		else{
			...
			$this->db->insert('module_access',$one_data);
		}
	 ...
	 * @author Mathew
	 **/
	public function login($identity, $password, $remember=FALSE,$login_type='',$method_name='')
	{ 
	    
	    
	   if($method_name=='login_employee'){
	     
	       $res = $this->login_employee($identity, $password, $remember=FALSE,$login_type='',$method_name);
...
// echo $login_type; exit;
		$this->trigger_events('extra_where');
		$this->db->select($this->identity_column . ',username, email,role, customer_user_id,customer_id,password, active, last_login,menu_access,supervisor,is_haccap_user');
		                  $this->db->where('email', $this->db->escape_str($identity));
		                  //$this->db->where('role', $login_type); 
		                  $this->db->limit(1);
		...
		if ($query->num_rows() === 1)
		{
		 ...
		 $new_query=$this->db->select('group_id')
		                  ->where('user_id',$uid)
		                  ->limit(1)
		                  ->get($this->tables['users_groups']);
		 
		 $new_user = $new_query->row();
		 
		 $group_id=$new_user->group_id;
		 
		
         $group_query=$this->db->select('*')
		                  ->where('id',$group_id)
		                  ->limit(1)
		                  ->get($this->tables['groups']);
		 
		 $group_user = $group_query->row();
		 $group_name=$group_user->name;
        $clearance_level=$group_user->clearanceLevel;
		$password = $this->hash_password_db($user->customer_user_id, $password);
			...
			if ($password === TRUE)
			{
				...
				$groups = $this->ion_auth->get_users_groups($user->customer_user_id)->result();
				foreach($groups as $group){
					$role = $group->id;
				}
				...
				$this->session->set_userdata('clearance_level', $clearance_level);
				...
				$this->session->set_userdata('customerId', $user->customer_user_id);
				$this->session->set_userdata('UserId', $uid);

				$this->update_last_login($user->customer_user_id);

				$this->clear_login_attempts($identity);

				if ($remember && $this->config->item('remember_users', 'ion_auth'))
				{
					$this->remember_user($user->customer_user_id);
				}
	...
	public function login_employee($identity, $password, $remember=FALSE,$manager='')
	{
		...
		$this->db->select($this->identity_column . ',first_name, email,role, emp_id,password, status,is_haccap_user');
		                  $this->db->where('email', $this->db->escape_str($identity));
		                  $this->db->limit(1);
		...
		if ($query->num_rows() === 1)
		{
			...
			$password = $this->hash_password_db($user->emp_id, $password,'login_employee');
			...
			if ($password === TRUE)
			{
			 ...
			 $this->session->set_userdata('customerId', $uid);
			 $this->session->set_userdata('UserId', $uid);
             $this->clear_login_attempts($identity);
	...
	public function adminlogin($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, email, customer_user_id, password, active, last_login,is_haccap_user')
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select(array(
			    $this->tables['users'].'.*',
			    $this->tables['users'].'.customer_user_id as id',
			    $this->tables['users'].'.customer_user_id as user_id'
			));
		}
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				...
				$this->db->join(
				    ...
				    $this->tables['users_groups'].'.'.$this->join['users'].'='.$this->tables['users'].'.customer_user_id',
				    'inner'
				);
	...
	public function user($id = NULL)
	{
		...
		$this->limit(1);
		$this->where($this->tables['users'].'.customer_user_id', $id);
		$this->users();
		return $this;
	}
	...
	public function update($id, array $data)
	{
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['users'], $data, array('customer_user_id' => $user->customer_user_id));
	...
	**/
	public function delete_user($id)
	{
		...
		$this->db->delete($this->tables['users'], array('customer_user_id' => $id));
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['users'], array('last_login' => time()), array('customer_user_id' => $id));
	...
	public function set_session($user)
	{
      ...
      if(isset($user->customer_user_id)) { 
       ...
       if(isset($user->supervisor) && $user->supervisor) { 
		...
		$session_data = array(
		    ...
		    'username'             => $uname,
		    'email'                => $user->email,
		    'is_haccap_user'       => $user->is_haccap_user,
		    'supervisor'           => $supervisor,
		    'user_id'              => $customer_user_id, //everyone likes to overwrite id so we'll use user_id
	...
	public function remember_user($id)
	{
		...
		$this->db->update($this->tables['users'], array('remember_code' => $salt), array('customer_user_id' => $id));
	...
	public function login_remembered_user()
	{
		...
		//get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', customer_user_id, username, email, last_login')
	...
	**/
	...
	public function get_edit($id,$i=null){
			$this->db->select('*');
	...
	public function insert_edit($data){
		$this->db->update('partner_users',$data);
	    return $this->db->insert_id();
	}
	
	//get module clearence level
	public function getModuleClrLevel($module,$i=null){
		$this->db->select('clearanceLevel');
		$this->db->from('modules');
		$this->db->where('moduleName',$module);
		$query=$this->db->get();
	...
	public function getGroupUrl($groupId,$i=null){
		$this->db->select('url');
		$this->db->from('partner_groups');
	...
	public function getBranchesDetails($customer_id,$i=null){
		$this->db->select('*');
		...
		if($errors['code'] != 0){
			...
			sleep(5);
			$this->getBranchesDetails($customer_id,$i);
		}else{
			$result = $query->result();
		    return $result;
		}
    ...
    public function getMenusdynamic($i=null){
	    
       
		$this->db->select('menu_access');
		$this->db->from('customer_users');
		$this->db->where('customer_user_id',$this->session->userdata('user_id'));
		...
		if($errors['code'] != 0){
			...
			sleep(5);
			$this->getMenusdynamic($i);
		}else{
		    
			$results = unserialize($query->result()[0]->menu_access);
		$menu_array= array();	
			
	foreach($results as $menuid){
	    $menu_array[] = $this->getMenusdesc($menuid);
	}
	...
	public function getMenusdesc($menuid,$i=null){
		...
		$this->db->where('is_haccap_menu','1');
		$query=$this->db->get();
	...
	public function getMenus($i=null){
...
// 		exit;
	...
	public function getSubMenus($menu_id,$i=null){
		$this->db->select('*');
		$this->db->from('menu_subitems');
	...
	public function checkMenuLevel($controller,$i=null){
		$this->db->select('*');
		$this->db->from('menu_items');
	...
	public function checkSubMenuLevel($controller,$i=null){
		$this->db->select('*');
		$this->db->from('menu_subitems');
		$this->db->where('controller',$controller);
		$query = $this->db->get();
	...
	public function getSubscription($customerId,$i=null){
		$this->db->select('*');
	...
	public function email_verified_password($new,$id){
		$salt       = $this->store_salt ? $this->salt() : FALSE;
	...
	public function update_pass($new,$id,$table_name=''){
	  ...
	  if($table_name=='employee'){
	      ...
	      $this->db->where('email',$user_email[0]->email);
	   	  $this->db->update('customer_users', $data);   
	  }
	...
	public function group_insert($data){
      $this->db->insert('customer_users_groups',$data);
	}
	public function user_update($data2,$id){
      $this->db->where('customer_user_id',$id);
	  $this->db->update('customer_users', $data2);
	}
