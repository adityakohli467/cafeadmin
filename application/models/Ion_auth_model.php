...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function hash_password_db($id, $password, $use_sha1_override=FALSE, $i=null)
	{
		...
		if($errors['code'] != 0){
			...
			sleep(5);
			$this->hash_password_db($id, $password, $use_sha1_override=FALSE,$i);
		}else{
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email, $package, $additional_data = array(), $groups = array())
	{
		...
		$packageAddons = $this->getAddons($package);
		
		$addons = $packageAddons[0];
		$pdate = date('Y-m-d H:i:s');
		$expDate = date('y-m-d', strtotime("$pdate +14 days"));
		//package update
		$customer_package = array(
			'customer_id' => $cust_id,
			'package_id' => $package,
			'no_of_users' => $addons->users,
			'u_amount' => $addons->u_amount,
			'user_updated_on' => $pdate,
			'no_of_branches' => $addons->branches,
			'b_amount' => $addons->b_amount,
			'branch_updated_on' => $pdate,
			'no_of_orders' => $addons->orders,
			'o_amount' => $addons->o_amount,
			'order_updated_on' => $pdate,
			'expiry' => $expDate,
			'base_amount' => $packageDetails[0]->amount,
			'status' => 'Trail'
		);
		
		$this->db->insert('customer_subscription_relation', $customer_package);
...
public function emp_register($first_name,$last_name,$username, $password,$user_type,$active,$mobile,$res_id,$Specilized_category, $additional_data = array(), $groups = array())
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ',username, email, customer_user_id,customer_id, password, active, last_login,is_outlet_manager')
...
// 			echo "<pre>";print_r($user);exit;
         ...
         $user_group_id=$group_user->id;
			...
			if ($password === TRUE)
			{
				...
				$this->session->set_userdata('clearance_level', $clearance_level);
                                ...
                                $this->session->set_userdata('user_group_id', $user_group_id);
				...
				$this->session->set_userdata('customerId', $user->customer_id);
				$this->session->set_userdata('is_outlet_manager', $user->is_outlet_manager);
	...
	**/
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    ...
		    'email'                => $user->email,
		    'user_id'              => $user->customer_user_id, //everyone likes to overwrite id so we'll use user_id
	...
	**/
	...
	public function getMenus($i=null){
		...
		$this->db->where('is_haccap_menu','0');
