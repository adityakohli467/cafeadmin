...
.dataTables_filter label {
	...
	<div class="col-md-12 page-head border-bottom">
		...
		<div class="btn-div col-md-3">
			<a href="<?php echo base_url(); ?>index.php/orders/standingOrders">
							...
							<tr class="tr">
								...
								<td class="text-left">
									<div class="handle-counter">
									  <button class="counter-minus btn abc" onclick="counterMinus(this,`<?php echo $items->itemId;?>`,`<?php echo $items->supplierId;?>`,`<?php echo number_format($items->price,2,'.',''); ?>`,`<?php echo $items->supplier_name; ?>`,`<?php echo $items->itemName; ?>`)" >-</button>
									  <input class="order" type="text" value="<?php echo $order_qty; ?>" id="<?php echo $items->itemId;?>" onchange="addItemToCart(this,`<?php echo $items->itemId;?>`,`<?php echo $items->supplierId;?>`,`<?php echo number_format($items->price,2,'.',''); ?>`,`<?php echo $items->supplier_name; ?>`,`<?php echo $items->itemName; ?>`);">
									  <button class="counter-plus btn abc" onclick="counterAdd(this,`<?php echo $items->itemId;?>`,`<?php echo $items->supplierId;?>`,`<?php echo number_format($items->price,2,'.',''); ?>`,`<?php echo $items->supplier_name; ?>`,`<?php echo $items->itemName; ?>`)">+</button>
									</div>
								</td>
							</tr>
						<?php }} ?>
		...
		<div class="col-md-4 col-sm-4">
			...
			<div class="panel">
				<div class="panel-body" style="padding-top:0px;padding-bottom:0px;">	
			    	...
			    	<div>
			    		...
			    		<textarea class="form-control" rows="3" name="comments" id="comments" placeholder="Delivery Instructions"></textarea>
			    	</div>
			    	<div class="row">
			    		<div class="col-md-6">
			    		...
			    		<div class="col-md-6">	
				    		...
				    		<!--</select>-->
				    		<input type="text" class="form-control datepicker" name="delivery_date" id="delivery_date" value="<?php echo date('d-m-Y');?>">
			    		</div>
			    	</div>
			    	<br>
			    	</div>
			...
			<div class="panel" style="background-color:#236ea6;">
				<div class="panel-body" style="padding-top:15px;padding-bottom:35px;">
			    	<div class="row">
			    		<div class="col-md-12">
				    		<label style="color:#fff;" for="supp_email" class="order-padding">Email</label>
			    	...
			    	<button class="btn btn-success button-width btncolor" onclick="placeOrder()" >Send</button>
			    	</div>
		...
		$.each( items_loop, function( key, value ) {
			...
			var itemPrice2 = Number(itemPrice1).toFixed(2);
			   ...
			   items+='<div class="item-row padding-row-big" style="margin-left:-25px;margin-right:-25px;"><div class="text-wrap padding-tb padding-sides">'+orderItems.itemName[key]+'</div>\n'
				    		...
				    		+'<div class="item-right3"><a onclick="deleteCartItem(`'+value+'`,`'+supId+'`,`'+itemPrice2+'`,`'+supp_name+'`,`'+orderItems.itemName[key]+'`)"><span class="glyphicon glyphicon-trash"></span></a></div>\n'
	...
	function placeOrder(){
		...
		if(balance > 0){
			if(budget_balance >= Number(order_total) && branch_budget >= Number(order_total)){
				$.ajax({
					type: "POST",
			        url: "<?php echo base_url();?>index.php/orders/placeNewOrders",
			        data:'supplier_id='+supplier+'&order='+json+'&delivery_date='+delivery_date+'&comments='+comments+'&order_date='+order_date+'&supp_email='+supp_email+'&supp_cc='+supp_cc,
			        success: function(data){
			        	alert(data);
			        	if(data == 'success'){
			        		var msg = '<div class="hideMe"><p class="alert alert-success">Order placed successfully</p></div>';
