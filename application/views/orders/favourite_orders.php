		...
		<div class="col-md-12">
			<div style="box-shadow:none;" class="panel panel-default">
				...
				<div class="panel-body">
					<table class="row-border table-condensed" cellspacing="0" width="100%" id="dataTables-example">
						<thead>
							<tr>
						...
						foreach($orders as $row){
							if($row->order_status == 'Sent'){
								...
								?>
							<tr class="tr">
								<td class="text-center"><a href="<?php echo base_url();?>index.php/orders/favOrderDetails/<?php echo $row->order_id;?>" class="update_user_button"><?php echo date('d-m-Y',strtotime($row->order_date)); ?></a></td>
								<td class="text-center"><a href="<?php echo base_url();?>index.php/orders/favOrderDetails/<?php echo $row->order_id;?>" class="update_user_button"><?php echo $row->order_number; ?></a></td>
								<td class="text-left"><?php echo $row->supplier_name; ?></td>
								<td class="text-right">$<?php echo number_format($row->order_total, '2','.',''); ?></td>
							    <td class="text-center"><a type="button" onClick="delete_row(<?php echo  $row->order_id; ?>);"><span class="glyphicon glyphicon-remove"></span></a></td>
							</tr>
						<?php }} ?>
							</tbody>
					</table>
	...
	if(confirm('Are you sure you want to remove from favourite orders')){
		    $.ajax({
		type: "POST",
        url: "<?php echo base_url();?>index.php/orders/fav_order_update",
        data:'id='+str,
