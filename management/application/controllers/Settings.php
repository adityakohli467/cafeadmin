		...
		if (!$this->ion_auth->logged_in()) {
			...
			if($user_limit >= $active_count){
				...
				if($customer_user_ids != ''){
					foreach($customer_user_ids as $key => $userId){
						if($userId != ''){
							
							$branch_update = $this->settings_model->edit_users_detail_row_details($key,$first_name,$last_name,$email,$status,$user_type,$branch_id,$customer_id);
							if($branch_update){
			...
			$newuser_count = count($_POST['new_first_name']);
			...
			if($balance > 0 && $balance >= $newuser_count-1){
				...
				if( $new_first_names != ''){
				    foreach($new_first_names as $res => $new_user_name){
						if($new_user_name != ''){
							if($new_email[$res] == '' || $new_user_type[$res] == '' || empty($new_branch_id[$res])){
								$this->session->set_flashdata('error_msg', 'Please fill all required fields');
							}else{
								$add_branches = $this->settings_model->add_users($new_first_names[$res],$new_last_name[$res],$new_email[$res],$new_user_type[$res],$new_branch_id[$res],$customer_id,$password);
								if($add_branches){
