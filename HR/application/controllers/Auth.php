...
class Auth extends CI_Controller {
    ...
    //log the user in
       function login($login_type="") {
        ...
        if ($this->form_validation->run() == true) {
            ...
            //check who is trying to login either admin,emplolyee,manager
          
           if($login_type =="employee"){
            $method_name = "login_employee";
            }else{
            $method_name = "login";
             }
             
     
             if ($this->ion_auth->login($this->input->post('email'), $this->input->post('password'), $remember,$login_type,$method_name)) { 
                $group_id = $this->session->userdata('groupId');
              
                 if(isset($login_type) && $login_type =='timesheet'){
                  	$this->session->set_userdata('timesheet_login', 'true');
                ...
                if($group_id == '2'){
                    ...
                    redirect('general/index', 'refresh');
                }
                
            } else {

                $this->session->set_flashdata('message', $this->ion_auth->errors());
                //show the error in popup
                redirect('auth/error_login', 'refresh');
   ...
   function error_login(){
       ...
       $data['link'] =  base_url()."index.php/auth/homepage";
      $this->load->view('general/header_general');
        $this->load->view('auth/error_login',$data);
   }
    function admin() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->ion_auth->adminlogin($this->input->post('identity'), $this->input->post('password'))) {
                ...
                $this->session->set_flashdata('message', $this->ion_auth->errors());
                redirect('auth/admin', 'refresh'); //use redirects instead of loading views for compatibility with MY_Controller libraries
            }
        } else {
            ...
            $this->load->view('header');
            $this->load->view('auth/admin', $this->data);
            //$this->load->view('footer');
        }
    }
    ...
    function logout() {
        ...
        //log the user out
        $this->session->unset_userdata('email');
        $this->session->unset_userdata('clearance_level');
			 ...
			 $this->session->unset_userdata('UserId');
			  $this->session->unset_userdata('timesheet_login');
        $logout = $this->ion_auth->logout();
        echo '<script>window.localStorage.removeItem("formSubmitted");</script>';
        //redirect them to the login page
        $this->session->set_flashdata('message', $this->ion_auth->messages());
        redirect('auth/homepage', 'refresh');
    }
    ...
    function change_password() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->data['old'] = $this->form_validation->set_value('old');
            $this->data['new'] = $this->form_validation->set_value('new');
            $this->data['cnf_password'] = $this->form_validation->set_value('new_confirm');
          ...
          function forgot_password_hr(){
            ...
            $identity = $this->db->where('email', $this->input->post('email'))->limit('1')->get('employee')->row();
            ...
            if ($forgotten) {
                ...
                $this->session->set_flashdata('message', 'The reset link has been sent to your email');
                redirect("auth/forgot_password/show_message", 'refresh'); //we should display a confirmation page here instead of the login page
            } else {
    ...
    public function reset_password($code = NULL) {
        ...
        if ($user) {
            ...
            if(!isset($user->id)){
              $user->id = $user->emp_id;  
              // so that we can recoznise on model page that we have to update employee table not the user table
              $type_of_user = "emp";
              $this->session->set_userdata('reset_password', $type_of_user);
            }else{
               $type_of_user = ''; 
            }
            ...
            $this->form_validation->set_rules('new_pwd', $this->lang->line('reset_password_validation_new_password_label'), 'required|min_length[' . $this->config->item('min_password_length', 'ion_auth') . ']|max_length[' . $this->config->item('max_password_length', 'ion_auth') . ']|matches[new_confirm]');
            ...
            if ($this->form_validation->run() == false) {
               ...
               // $this->load->view('footer');
                ...
                if ($this->_valid_csrf_nonce() === FALSE || $user->id != $this->input->post('user_id')) {
                    ...
                    $change = $this->ion_auth->reset_password($identity, $this->input->post('new_pwd'));
            ...
            $this->session->set_flashdata('message', 'This email reset link has been expired. Please request a new reset link by clicking on forgot password? from your employee portal    <a href="'.base_url('index.php/auth/login/employee').'">login</a>');
            redirect("auth/forgot_password", 'refresh');
        }
    }
    ...
    function create_user() {
        ...
        if ($this->form_validation->run() == true && $this->ion_auth->register($username, $password, $email)) {
            ...
            $data['password_confirm'] = $this->form_validation->set_value('password_confirm');  
    ...
    public function registration(){
        ...
        if ($this->form_validation->run() == true && $this->ion_auth->register($username, $password, $email, $package)) {
            //check to see if we are creating the user
            //redirect them back to the admin page
            ...
            //subscription plans
            $packages = $this->ion_auth_model->getPackages();
	...
	function insert_edit(){
		...
		$some=$this->ion_auth->insert_edit($data);
    ...
    function _render_page($view, $data = null, $render = false) {
      ...
      //  $view_header = $this->load->view('', $data);
       $view_content = $this->load->view($view, $this->viewdata, $render);
      //  $view_footer = $this->load->view('footer', $render);
        $view_html =  $view_content ;
    ...
    public function email_check(){
        $email = $_POST['email'];
        $res = $this->ion_auth->email_check($email);
        //print_r($res);
        if($res){
            echo 'false';
        }else{
