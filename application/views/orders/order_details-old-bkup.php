		...
		if(balance > 0){
			if(budget_balance >= Number(order_total) && branch_budget >= Number(order_total)){
				$.ajax({
			        ...
			        data:'supplier_id='+supplier+'&order='+json+'&delivery_date='+delivery_date+'&comments='+comments+'&order_id='+order_id+'&order_date='+order_date+'&supp_email='+supp_email+'&supp_cc='+supp_cc+'&order_number='+order_number,
			        success: function(data){
			        	alert(data);
			        	if(data == 'success'){
