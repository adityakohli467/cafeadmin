...
// 		$this->db->join('menuPlanner_category', 'menuPlanner_category.category_id = menuPlanner.category');
		...
		$this->db->order_by($idFieldname,"DESC");
		$query = $this->db->get();
	
		return $query->result();
	}   
	public function getGraphData($branch_id){
        // echo $table_name;
        $this->db->select_sum("total_income");
        $this->db->select("date,month");
		$this->db->from('eod_graph_report');
...
// 		$this->db->join('menuPlanner_category', 'menuPlanner_category.category_id = menuPlanner.category');
	...
	function updateDataUsingAnotherCol($table_name,$data,$whereid,$whereCol){
		 
		 $this->db->where($whereCol,$whereid);
		 return $this->db->update($table_name,$data);
	}
	...
	function recordDeleteUsingAnotherCol($whereid,$whereCol,$table_name){
