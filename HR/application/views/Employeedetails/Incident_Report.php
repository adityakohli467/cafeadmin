<div class="row item">
			...
			<style>
@media print {
    #mainHeader,.page-head,#footer,#contact_submit,.btn-cancel,.site-mobile-menu {
...
@media print {
    ...
    /* Textarea never prints correctly, so hide it */
    textarea.form-control {
        display: none !important;
    }

    /* Show this block only when printing */
    .print-value-text {
...
/* Hide print-only value in normal screen */
.print-value-text {
    display: none;
}
</style>
	...
	<div  class="col-md-12 page-head border-bottom">
        ...
        class="btn btn-danger btn-ph"
        onclick="window.location.href='<?php echo base_url(); ?>index.php/Employeedetails/view_Incident_Report'">
    Back
</button>
</div>
		<div class="col-md-3">
			<button class="btn btn-success btn-ph" id="printBtn" onclick="window.print()" type="button">Print Report</button>
</div>
...
<div class="container-fluid main-container">
  			...
  			<div class="col-md-12">
	        	...
	        	<div class="panel pn">
			        ...
			        <div class="panel-heading">
	          		...
	          		<div class="panel-body incident-form ct-form-in">
	          			...
	          			<div class="form-group">
							...
							<?php  }else {  ?>
							<input type="hidden" name="emp_id" value="<?php echo $loggedn_emp_id; ?>">
								<?php  } ?>
					</div>
						</div>	
					...
					<h3 class="panel-title">Personal details (of injured):	 : </h3>
						...
						<div class="form-row">
						<div class="form-group col-sm-6" style="flex-wrap: wrap;">
						    <label class="col-lg-2 control-label">Gender:</label><br>
					<label class="radio-inline">
					 ...
					 &nbsp;&nbsp;
					  ...
					  <label class="col-lg-2 control-label">Type:</label><br>
					  <label class="radio-inline">
				...
				<input type="date" class="form-control datetime"  name="incident_date"  autocomplete="off" required  value="<?php if(isset($details[0]->incident_date)){ echo $details[0]->incident_date; } ?>">
					...
					<label for="abn" class="col-lg-4 fix-width-textarea control-label">Where did the incident occur? (Please specify)</label>
					...
					<div class="print-value-text">
    <?php if(isset($details[0]->incident_place)){ echo nl2br($details[0]->incident_place); } ?>
</div>
					</div>
					
					<div class="form-group col-md-12">
					<label for="incident_detail" class="col-lg-5 fix-width-textarea control-label"> What was the nature of, and injury resulting from, this incident? </br><small>(Please explain in your own words what had happened)</small> </label>
				   ...
				   <div class="print-value-text">
    <?php if(isset($details[0]->incident_detail)){ echo nl2br($details[0]->incident_detail); } ?>
</div>
    ...
    <label for="action_to_take" class="col-lg-5 control-label">
        Describe what has been done to resolve the hazard/incident:
    </label>

    <!-- Editable textarea (screen only) -->
    <textarea class="form-control" name="action_to_take" rows="3" autocomplete="off"><?php
        if (isset($details[0]->action_to_take)) { 
            echo $details[0]->action_to_take; 
        }
    ?></textarea>
    ...
    <div class="print-value-text">
        <?php
            if (isset($details[0]->action_to_take)) { 
                echo nl2br($details[0]->action_to_take); 
            }
        ?>
    </div>
</div>
			...
			<button type="submit" name="contact_submit" id="contact_submit" class="btn btn-success btn-ph">SUBMIT</button>
					<a href="<?php echo base_url(); ?>index.php/Employeedetails/view_Incident_Report">
						<button type="button"  class="btn btn-ph btn-ph-cancel btn-success btn-cancel">CANCEL</button>
					</a>
		</div>
			</div>
