   ...
   $query = $this->db->select('password, salt')
		...
		if($errors['code'] != 0){
			...
			else
			{
				...
				// echo "DB0=  ".$db_password; echo "</br>";
			}

// 	echo "HPD".$hash_password_db->password;
// 	exit;
...
// echo $login_type; exit;
		$this->trigger_events('extra_where');
		$this->db->select($this->identity_column . ',username, email,role, customer_user_id,customer_id,password, active, last_login,menu_access,supervisor');
...
//  echo $user->customer_user_id; exit;
	...
	public function login_employee($identity, $password, $remember=FALSE,$manager='')
	{
		...
		$this->db->select($this->identity_column . ',first_name, email,role, emp_id,password, status');
		                  $this->db->where('email', $this->db->escape_str($identity));
		                  $this->db->limit(1);
	...
	public function adminlogin($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, email, customer_user_id, password, active, last_login')
