...
class Settings_model extends CI_Model{
	...
	function get_users_admin($i=null){
		$user_id = $this->session->userdata('customerId');
		$branch_id = $this->session->userdata('branch_id');
        $query = $this->db->query("SELECT customer_users.*,customer_users_groups.group_id,customer_groups.name FROM customer_users LEFT JOIN customer_users_groups ON customer_users.customer_user_id=customer_users_groups.user_id LEFT JOIN customer_groups ON customer_users_groups.group_id = customer_groups.id  WHERE customer_users.customer_id='".$user_id."' AND group_id <> 7");
